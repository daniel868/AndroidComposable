...
class HeroCacheServiceImpl(
    ...
    override suspend fun insertAll(heros: List<Hero>) {
        for (currentHero in heros) {
            try {
                insert(currentHero)
            } catch (e: Exception) {
                e.printStackTrace()
    ...
    override suspend fun searchByAttackType(attackType: String): List<Hero> {
        return queries.searchHeroByAttackType(attackType).executeAsList().map {
            it.toHero()
        }
    }
